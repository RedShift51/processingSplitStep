...
halfs = [18.0, 127.5]
N = 36864#8192#16384 #8192#36864
dx = 3900. / 36864 #2 * 866.6666 / N #3900. / N
"""
...
def iteration(arr, mask, i_brd, o_brd, Iavg):
    ...
    return np.mean(ans), np.std(ans), len(ans) #data
...
def slices_stats(screen, dd, dist_curr, interp_func, rads):
    ...
    # Hardcode
    N = 36864#8192 #36864 #2048#8192#16384 #8192 #36864
    dx = 3900. / 36864. #2 * 866.6666 / N
    ...
    means, stds, count = [k[0] for k in list(result)], [k[1] for k in list(result)], \
                        [k[2] for k in list(result)]
...
def main():
    ...
    #path_dir = "/home/abuzovkin/atmosphere/screens/36_small/scr"
    ...
    for d0,d in enumerate(dists):
        ...
        for i0, fold in enumerate([m for m in os.listdir(path_dir)]):
            if os.path.exists(os.path.join("../big_data/stats/36864_eq_mean", \
        ...
        #direct_res = slices_stats(avg_arr)
            with open(os.path.join("../big_data/stats/36864_eq_mean", \
                    str(d)) + "_f" + str(fold), "wb") \
                    as ftemp:
                pkl.dump(direct_res, ftemp)
            #ftemp.write(direct_res)
            print(fold, d)
